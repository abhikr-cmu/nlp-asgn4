    ...
    for node in nodes:
        ...
        for n in node.children:
            if isinstance(n, element.Tag):
                if len(n.text) > 0 and 'property' not in n['class'] and n.name != 'a':
                    text += n.text
...
def parse_optional_args(sig):
    if '(' in sig and ')' in sig:
        ...
        kwargs_combinations = []
        kwargs = kwargs[:min(len(kwargs), 25)]
        # pdb.set_trace()
...
if __name__ == '__main__':
    fd = open('pandas-docs_final.jsonl', 'w', encoding='utf-8')
    counter = 0
    module_counter = {}
    fileset = []
    for root, dirs, files in os.walk("pandas_generated"):
        for file in files:
            if file.endswith('.html'):
                fileset.append(file)

    fileset.sort()
    files_len = len(fileset) 
    i = 1
    for file in fileset:
        print(i, "/", files_len, ":", file)
        i += 1
        input_filename = os.path.join(root, file)
        module_count = 0
        with open(input_filename, encoding='utf-8') as html_file:
            soup = BeautifulSoup(html_file.read(), 'html.parser')
            listres = soup.find_all('div')
            for item in listres:
                if item is None:
                    continue
                all_sections = item.find_all('dl')
                current_class_name = None
                for section in all_sections:
                    section_class_attrs = section.get('class')
                    if section_class_attrs is None or len(section_class_attrs) <= 1:
                        continue
                    sig_type = section_class_attrs[1]
